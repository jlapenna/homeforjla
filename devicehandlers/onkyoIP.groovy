 ...
 *  https://github.com/allanak/SmartThings/blob/master/onkyoIP.groovy
 *
 *  Usage:
...
metadata {
    ...
    tiles {
        ...
        standardTile("cd", "device.switch", decoration: "flat") {
            state "cd", label: 'Audio', action: "cd", icon:"st.Electronics.electronics19"
        }
        standardTile("streambox", "device.switch", decoration: "flat") {
            state "streambox", label: 'Video', action: "streambox", icon:"st.Electronics.electronics18"
        }
    ...
    main "switch"
        details(["switch","mute","cd","streambox","net","levelSliderControl","zone2"])
...
def parse(description) {
    def msg = parseLanMessage(description)
        ...
        def data = msg.data              // => either JSON or XML in response body
	                                 //    (whichever is specified by content-type header in response)
}
...
def cd() {
    ...
    return new physicalgraph.device.HubAction(msg,physicalgraph.device.Protocol.LAN)
}

def streambox() {
    log.debug "Setting input to streambox"
